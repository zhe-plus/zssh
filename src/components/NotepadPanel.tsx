...
export function NotepadPanel({ open, onClose, lang, clipboardText }: NotepadPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[55] flex items-center justify-center bg-black/60">
      <div className="w-[960px] max-w-[95vw] h-[80vh] max-h-[85vh] flex flex-col rounded-lg bg-[var(--color-gray-900)] border border-[var(--color-gray-700)] shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--color-gray-800)] rounded-t-lg shrink-0">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="px-2 py-1 rounded text-xs text-[var(--color-gray-400)] hover:text-white hover:bg-[var(--color-gray-800)] flex items-center gap-1"
              title={viewMode === "split" ? t(lang_, "notepadEditOnly") : t(lang_, "notepadSplitView")}
            >
              {viewMode === "split" ? <Eye className="size-3.5" /> : <Edit3 className="size-3.5" />}
              {viewMode === "split" ? t(lang_, "notepadPreview") : t(lang_, "notepadEdit")}
            </button>
        ...
        <div className="flex-1 flex min-h-0">
          {/* Sidebar - File List */}
          <div className="w-48 shrink-0 border-r border-[var(--color-gray-800)] flex flex-col bg-[var(--color-gray-950)]">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            {/* Title + toolbar */}
            <div className="flex items-center gap-2 px-3 py-1.5 border-b border-[var(--color-gray-800)] shrink-0">
              <input
                type="text"
                value={activeNote?.title ?? ""}
                onChange={(e) => handleTitleChange(e.target.value)}
                placeholder={t(lang_, "notepadTitlePlaceholder")}
                className="flex-1 bg-transparent text-sm text-white outline-none placeholder:text-[var(--color-gray-600)]"
                disabled={!activeNote}
              />
              <div className="flex items-center gap-1">
                <button
                ...
                <>
                  ...
                  <div
                    className={[
                      ...
                      viewMode === "split" ? "w-1/2 border-r border-[var(--color-gray-800)]" : "w-full",
                    ].join(" ")}
                  >
                    <textarea
                      ref={editorRef}
                  ...
                  {/* Preview */}
                  {viewMode === "split" && (
                    <div className="w-1/2 overflow-auto p-3" ref={previewRef}>
                      <div
